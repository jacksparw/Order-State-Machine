...
public class OrderService {
	...
	public Order create(Date when) {
		return this.orderRepository.saveAndFlush(new Order(when, OrderStates.SUBMITTED));
	}
					...
					sma.addStateMachineInterceptor(new StateMachineInterceptorAdapter<OrderStates, OrderEvents>() {
						...
						public void preStateChange(State<OrderStates, OrderEvents> state,
							...
							Optional.ofNullable(message).ifPresent(
							        msg -> Optional.ofNullable(Long.class.cast(msg.getHeaders().getOrDefault(ORDER_ID_HEADER, -1L)))
                                    .ifPresent(orderId1 -> {
                                        Order updatedOrder = orderRepository.getOne(orderId1);
                                        updatedOrder.setOrderState(state.getId());
